...
interface AccountService : SerializeAsToken {
     ...
     */
    @Suspendable
    fun createAccount(name: String): CordaFuture<StateAndRef<ExampleAccounts>>
     ...
     *
     * @param id the account ID to return the [ExampleAccounts] for.
     */
    fun accountInfo(id: UUID): StateAndRef<ExampleAccounts>?

    /**
     * Returns the [ExampleAccounts] for an account specified by [owningKey]. This method will return accounts created on
     ...
     *
     * @param owningKey the public key to return an [ExampleAccounts] for.
     */
    fun accountInfo(owningKey: PublicKey): StateAndRef<ExampleAccounts>?

    /**
     * Returns the [ExampleAccounts]s for an accounts specified by [name]. The assumption here is that Account names are
     * unique at the node level but are not guaranteed to be unique at the network level. The host [Party], stored in
     * the [ExampleAccounts] can be used to de-duplicate accountInfo names at the network level. Also, the accountInfo ID is
     * unique at the network level.
     *
     * This method may return more than one [ExampleAccounts].
     *
     * @param name the account name to return [ExampleAccounts]s for.
     */
    fun accountInfo(name: String): List<StateAndRef<ExampleAccounts>>

    /**
     * Shares an [ExampleAccounts] [StateAndRef] with the specified [Party]. The [ExampleAccounts]is always stored by the
